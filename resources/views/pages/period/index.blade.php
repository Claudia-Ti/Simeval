              ...
              <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                  <div class="card-header">
                    @if($user->level=='Admin')
                  <a href="{{url('/period/create')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah</a>&nbsp;
                  @endif
                    <h4>{{$pageName}}</h4>
                    <div class="card-header-action">
                        
                      <form action="{{url('period_search')}}" method="POST">
                        @csrf
                        @php $filter = session('filter-period');@endphp
                        <div class="input-group">
                          <input type="text" class="form-control" placeholder="Search" name="name" value="{{$filter??''}}">
                          <div class="input-group-btn">
                            <button class="btn btn-primary"><i class="fas fa-search"></i></button>
                          </div>
                          @if($filter)
                          <div class="input-group-append">
                            <a href="{{url('period_clear')}}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                          </div>
                          @endif
                        </div>
                      </form>
                      ...
                      <table class="table table-bordered table-striped table-md data-table">
                       ...
                       @foreach($data as $d)
                            <tr>
                                ...
                                @if($user->level=='Admin')
                                <td style="width:10%;">
                                    <form method="post" action="{{url('period',$d->id)}}">
                                        <a href='{{url("/period/".$d->id."/edit")}}' class="btn btn-warning"><i class="fa fa-edit"></i></a>
