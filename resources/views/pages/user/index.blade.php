                ...
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            @if($user->level=='Admin')
                            <a href="{{url('/user/create')}}" class="btn btn-primary"><i class="fa fa-plus"></i>
                                Tambah</a>&nbsp;
                                @endif
                            <h4>{{$pageName}}</h4>
                            <div class="card-header-action">

                                @php $filter = session('filter-user');@endphp
                            <a class="btn btn-griff" data-toggle="collapse" href="#collapseExample" role="button"
                              aria-expanded="false" aria-controls="collapseExample">
                              <i class="fa fa-filter"></i> Filter
                          </a>
                          @if(isset($filter))
                                                                    <a href="{{url('/user_clear')}}"
                        ...
                        <div class="card-body">
                            ...
                            <div class="filter">
                                <div class="collapse" id="collapseExample">
                                    ...
                                    <form action="{{url('user_filter')}}" method="POST">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <select
                                                                    ...
                                                                    <option @if($filter && $filter['level'] && $filter['level']=="Admin") selected @endif>Admin</option>
                                                                    <option @if($filter && $filter['level'] && $filter['level']=='Kasat') selected @endif>Kasat</option>
                                                                    <option @if($filter && $filter['level'] && $filter['level']=='Unser Penilai') selected @endif>Unser Penilai / Wakapolres</option>
                                                                </select>
                                                                @error('level')
                                                                <div class="invalid-feedback">{{$message}}</div>
                                                                @enderror
                                    ...
                                    <tr>
                                        ...
                                        <td >
                                            <form method="post" action="{{route('user.destroy',$d->id)}}">
                                                <a href='{{url("/user/".$d->id."/edit")}}' class="btn btn-warning"><i
                                        ...
                                        <td>
                                            <form method="post" action="{{url('user/reset_password',$d->id)}}">
                                                @csrf


                                                <button type="submit" class="btn btn-primary show_confirm2"
                                                    data="{{$d->p_name}}"><i
                                                        class="fa fa-lock"></i>&nbsp;&nbsp;Reset</i></button>
