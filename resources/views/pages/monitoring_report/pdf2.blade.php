...
<body>
    <table style="width:100%">
        <thead>
            ...
            <tr><td><hr/></td></tr>
            <tr><td style="text-align: center"><h4>FORMULIR HASIL MONITORING KINERJA {{$pageName}}</h4></td></tr>
        </thead>
        <tbody>
            <tr><td >
                <table style="width:100%" id="myTable">
                    <tr><th width="5%">No.</th><th>Nama Pegawai</th><th>Nilai</th><th>Predikat</th></tr>
                        ...
                        @php $index=1;@endphp
                        @foreach($data as $d)
                            <tr><td>{{$index}}</td><td>{{$d["officerName"]}} : {{$d["officerPosition"]}} : {{$d["officerUnit"]}}</td>
                            <?php
                                $d["totalBobot"] = $d["totalBobot"]==0?1:$d["totalBobot"];
                                $value = (($d["totalPencapaian"]/$d["totalBobot"])*1.1)*100;
                            ?>
                            ...
                            <tr>
                                <td colspan='4' style="height: 100px;overflow:hidden">
                                    ...
                                    <b>Catatan untuk {{$d["officerName"]}}</b>
                                        ...
                                        @endforeach
                                    @elseif(count($d["notes"])==1)
                                        Kriteria : <b>{{$d["notes"][0]->kriteria}}, Catatan : {{$d["notes"][0]->notes}}</b>
                                    @else
